...
const resizedFacepicAttachmentsPath = `/home/twc2/camans/server/files/resized-facepic-attachments`;
...
const importAttachments = async () => {
  ...
  directories.forEach(directory => {
    ...
    files.forEach(file => {
      ...
      fs.copyFileSync(`workers/${directory}/${file}`, attachment.file_path);
  ...
  files.forEach(file => {
    ...
    im(`${facepicAttachmentsPath}/${file}`).resize(600, 800).write(`${facepicAttachmentsPath}/${file}`, async (err) => {
      ...
      if (facepicPathToId[path]) {
        ...
        console.log(`=== Resized facepic ${counter++} - facepic id - ${facepicPathToId[path]} ===`);
      }
    });
  })
